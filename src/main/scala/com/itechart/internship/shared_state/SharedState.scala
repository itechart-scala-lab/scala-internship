package com.itechart.internship.shared_state
...
import com.itechart.internship.shared_state.IosCommon.logger
...
object ExerciseZero extends IOApp {
  ...
  class SimpleRef[A](ar: AtomicReference[A]) extends IOAtomicRef[A] {
    ...
    override def update(f: A => A): IO[Unit] = IO(ar.set(f(ar.get())))
