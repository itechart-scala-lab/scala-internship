...
import com.itechart.internship.http.Protocol._
import org.http4s._
...
object HttpServer extends IOApp {
  ...
  private val multipartRoutes = HttpRoutes.of[IO] {
    ...
    // Implement HTTP endpoint that processes a multipart request. The request is expected to have
    // the following parts:
    ...
    BlazeClientBuilder[IO](ExecutionContext.global).resource
      ...
      .use { case (client, blocker) =>
        for {
          ...
          // Exercise 2. Call HTTP endpoint, implemented in scope of Exercise 1.
          // curl "localhost:9001/params/validate?timestamp=2020-11-04T14:19:54.736Z"
          _ <- printLine()

          _ <- printLine(string = "Executing request with headers and cookies:")
          _ <- client.expect[String](
            Method.GET(uri / "headers", Header("Request-Header", "Request header value"))
          ) >>= printLine

          // Call HTTP endpoint, implemented in scope of Exercise 2.
          ...
          _ <- {
            implicit val Encoder: EntityEncoder[IO, User] =
              EntityEncoder.stringEncoder[IO].contramap { user: User =>
          ...
          _ <- {
            ...
            client.expect[String](
              Method.POST(multipart, uri / "multipart").map(_.withHeaders(multipart.headers))
            ) >>= printLine
          }
          _ <- printLine()
        } yield ()
